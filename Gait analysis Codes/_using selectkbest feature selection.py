...
# Initialize an empty list to store selected features for each file
...
for filename in os.listdir(dataset_path):
  ...
  with open(os.path.join(dataset_path, filename), 'r', encoding='latin-1') as file:
      ...
      # Create dummy target variable with the same number of samples as signals
      dummy_y = np.zeros(signals.shape[0])
      
      # Fit SelectKBest selector on signals
      selector.fit(signals, dummy_y)
      
      # Get selected features indices
      selected_columns_indices = selector.get_support(indices=True)
      
      # Get selected features
      selected_features = signals[:, selected_columns_indices]
      
      # Append selected features to the list
      selected_features_list.append(selected_features)
      
      # Print selected columns for each file
      print(f"Selected columns for file {filename}: {selected_columns_indices}")

# Convert the list of selected features to a numpy array
selected_features_array = np.array(selected_features_list)

# Print the shape of the selected features array
print("Shape of selected features array:", selected_features_array.shape)
...
for filename in os.listdir(r"D:\Newfolder\s7\project\gait dataset\gait-in-parkinsons-disease-1.0.0\data"):
 with open(os.path.join(r"D:\Newfolder\s7\project\gait dataset\gait-in-parkinsons-disease-1.0.0\data", filename), 'r', encoding='latin-1') as file:
     ...
     for line in data:
         ...
         if len(linedata) >= 17:
             try:
                 sig1.append(float(linedata[11]))
                 sig2.append(float(linedata[12]))
                 sig3.append(float(linedata[13]))
                 sig4.append(float(linedata[14]))
                 
             except ValueError as e:
                 print(f"Error processing line in file {filename}: {e}")

     tempsig = [sig1, sig2, sig3, sig4,]
...
for j, patient_signals in enumerate(signals):
   for k, signal in enumerate(patient_signals):
       temp_coeffs = []
       for i in range(num_scales):
           try:
               coeffs, freq = pywt.cwt(signal, scales, wavelet, 1)
               temp_coeffs.append(coeffs)
           except ValueError as e:
               print(f"Error processing patient {j}, signal {k}, scale {i}: {e}")
               # If there's an error, fill with zeros
               temp_coeffs.append(np.zeros_like(signal))

       # Fill train_data with the coefficients
       for scale_idx, coeffs in enumerate(temp_coeffs):
           for a in range(len(coeffs)):
               for b in range(len(coeffs[0])):
                   train_data[j, a, b, scale_idx] = coeffs[a][b]
...
output_directory = r"D:\Newfolder\s7\project\spectrogram images\using selectkbest fsa\spect"
...
# Choose the output directory for saving spectrogram images
output_directory = r"D:\Newfolder\s7\project\spectrogram images\using selectkbest fsa\spect resized"
...
original_directory = r"D:\Newfolder\s7\project\spectrogram images\using selectkbest fsa\spect resized"

# Set the path to the resized images directory
resized_directory_224 = r"D:\Newfolder\s7\project\spectrogram images\using selectkbest fsa\spect 224"
...
# Example usage
data_path = r"D:\Newfolder\s7\project\spectrogram images\using selectkbest fsa\spect 224"
